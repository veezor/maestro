...
To utilize multiple web services follow the below steps:

1. Configure the **ECS_SERVICE_TASK_PROCESSES** variable with your additionals web services (like the below example).
``` bash
web{512;1024}:1-2[mem=60;cpu=80;alb=60];web1{256;512}:1-2[mem=80;cpu=80;alb=60]
```
The additionals web services must to follow name pattern: web1; web2; web3...

2. Configure in your project a json file with the listener's rule conditions to forward traffic for additionals web services (following this [pattern](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/elbv2/create-rule.html#examples)).

The config file need to be created in path: **config/maestro/elb-conditions/service_name.json**.
